...
export const validateClockInStaff: ValidationChain[] = [
  body("teacherId").isInt({ min: 1 }).withMessage("teacherId is required").toInt(),
  body("notes").optional({ values: "falsy" }).isString().withMessage("notes must be a string"),
];

export const validateClockOutStaff: ValidationChain[] = [
  body("teacherId").isInt({ min: 1 }).withMessage("teacherId is required").toInt(),
  body("notes").optional({ values: "falsy" }).isString().withMessage("notes must be a string"),
  timeField("timeOut"),
];    
...
export const validateCreateStaffAttendance: ValidationChain[] = [
  body("teacherId").isInt({ min: 1 }).withMessage("teacherId is required").toInt(),
  // body("status")
  //   .notEmpty()
  ...
  body("date").notEmpty().withMessage("date is required").withMessage("date must be a valid ISO date"),
  body("reason").optional({ values: "falsy" }).isLength({ max: 1000 }).withMessage("reason must not exceed 1000 characters"),
  timeField("timeIn"),
  timeField("timeOut"),
];

export const validateUpdateStaffAttendance: ValidationChain[] = [
...
export const validateStaffAttendanceId: ValidationChain[] = [
  param("id").isInt({ min: 1 }).withMessage("Attendance ID must be a positive integer").toInt(),
];

export const validateStaffAttendanceQuery: ValidationChain[] = [
...
export const  validateStaffAttendanceSummaryQuery: ValidationChain[] = [
  query("teacherId").optional({ values: "falsy" }).isInt({ min: 1 }).withMessage("teacherId must be a positive integer").toInt(), 
  query("startDate").optional({ values: "falsy" }).isISO8601().withMessage("startDate must be a valid ISO date"),
  query("endDate").optional({ values: "falsy" }).isISO8601().withMessage("endDate must be a valid ISO date"),
];
