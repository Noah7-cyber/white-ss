...
const SubjectRowActions: React.FC<ActionProps> = ({
  onView,
  onAddMilestone,
  onAddAssessment,
  onDelete,
  ...
  const defaultActions = [
    ...
    { label: "Add Milestone", action: onAddMilestone },
    { label: "Add Assessment", action: onAddAssessment },
    { label: "Delete", action: onDelete, isDanger: true },
  ];

  const availableActions = customActions?.length ? customActions : defaultActions;
    ...
    <>
      ...
      <Box className="!p-0 relative z-[1001] " onClick={(e) => e.stopPropagation()}>
        ...
        <Popover
          ...
          PaperProps={{
            className:
              "max-w-[240px] w-[160px] !p-4 !rounded-lg !shadow-[0_8px_24px_rgba(0,0,0,0.4)] ml-4",
            style: { zIndex: 1100 },
          }}
...
export default SubjectRowActions;
