...
import { useResendResetPassword } from "./hook/useResendResetPassword";
import { Button } from "@/modules/shared/component/Button";
import NextLink from "next/link";
import { AuthRoutes } from "@/routes/auth.routes";
import { useSearchParams } from "next/navigation";

const ResendResetPasswordForm = () => {
  const { control, handleSubmit, isPending } = useResendResetPassword();
      ...
      >
        ...
        <Box className="text-center space-y-1">
          ...
          <Typography variant="body2" className="text-primary-outlined !font-medium">
            Enter your email to receive a new password reset token
          </Typography>
        </Box>
        ...
        >
          {isPending ? "Sending..." : "Resend Reset Token"}
        </Button>

        {/* Back to Login */}
        <Box className="text-center">
          <Link component={NextLink} href={loginHref} className="!text-sm !text-primary-outlined">
            Back to Login
          </Link>
        </Box>
      </form>
...
export default ResendResetPasswordForm;
