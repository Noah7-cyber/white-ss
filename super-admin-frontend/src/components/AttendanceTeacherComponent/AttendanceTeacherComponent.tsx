...
import { useAttendanceTeacher, type BaseRow } from "./hooks/useAttendanceTeacher";
...
interface AttendanceTeacherComponentProps {
  role: "admin" | "staff";
}
export const AttendanceTeacherComponent = ({ role }: AttendanceTeacherComponentProps) => {
  const {
  ...
  const headers = getHeaders("teacher");
      ...
      <div className="flex flex-col flex-1 gap-4 md:gap-6">
        ...
        <div className="md:hidden flex flex-col gap-3">
          {(attendances ?? []).map((row: BaseRow) => {
            ...
            const href = tid ? `${attendanceBase}/${tid}/teacher` : "#";
            const first = row?.teacher?.user?.firstName ?? "";
            const last = row?.teacher?.user?.lastName ?? "";
