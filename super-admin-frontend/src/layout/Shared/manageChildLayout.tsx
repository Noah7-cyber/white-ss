...
export default function ManageChildLayout({ children }: { children: React.ReactNode }) {
    ...
    <div className="w-full mx-auto">
      ...
      <div className="p-4 mx-4 bg-white rounded-2xl overflow-hidden">
        <ScrollableTabBar innerClassName="flex flex-nowrap items-center gap-2 min-w-min">
          {tabs.map((tab) => {
            const href = `${DashboardRoutes.addChildren}/${tab.id}`;
            ...
            return (
              <Link
                key={tab.id || "profile"}
                href={href}
                className={`shrink-0 whitespace-nowrap px-6 py-3 rounded-full text-sm font-light transition-colors ${
                  isActive
