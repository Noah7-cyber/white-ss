...
interface LogBathroomModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const LogBathroomModal: FC<LogBathroomModalProps> = ({ isOpen, onClose }) => {
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="rounded-lg w-[440px]">
      <Box className="flex flex-col gap-6">
        <Box className="flex flex-row items-center justify-between pb-5 border-b border-solid border-border-light">
          <Typography className="font-bold! text-sm!">Log Bathroom Time</Typography>
        ...
        <Box className="flex flex-col gap-6">
          ...
          <Box className="flex flex-row gap-6 items-end">
            <Dropdown
              ...
              hasSearch
            />
            <TextField endIcon={<ClockIcon />} label="Time" labelOnTop />
          </Box>
