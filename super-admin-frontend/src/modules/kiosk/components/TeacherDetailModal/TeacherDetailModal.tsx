...
import useTeacherDetailModal from "./hooks/useTeacherDetailModal";
...
const TeacherDetailModal: React.FC<TeacherDetailModalProps> = ({
  ...
  const { notes, setNotes, isRecording, handleAction, handleClose, formatLastClockIn } = useTeacherDetailModal({
    teacher,
    isClockedIn,
    onClose,
    onClockIn,
      ...
      <Box className="flex max-h-[85dvh] flex-col gap-4 overflow-y-auto p-4 sm:p-5">
        ...
        <Box className="flex items-center justify-between gap-2 border-b border-gray-200 pb-3">
          ...
          <Typography className="line-clamp-2 flex-1 text-center text-sm! font-semibold! text-gray-800! sm:text-base!">
            {isClockedIn ? `Clock Out for ${teacher.name}` : `Clock In for ${teacher.name}`}
          </Typography>
          <button
            onClick={handleClose}
            className="cursor-pointer"
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Typography className="flex items-start gap-1 text-sm! font-medium! sm:items-center sm:text-right">
            <span>Status</span> <span>{getStatusBadge(teacher.status)}</span>
          </Typography>
        </div>
        ...
        {!isClockedIn ? (
          <Box className="flex flex-col gap-2">
            ...
            <textarea
              ...
              placeholder="Add any notes (e.g, late arrival, special instructions)..."
